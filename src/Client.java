...
public class Client {
	public static void main(String[] args) {
		...
		System.out.println("Bonjour ! Bienvenue dans l'application VEPICK :"
				...
				+ "\n2 : Emprunter un v√©lo ou R√©server un v√©lo" + "\n3 : Rendre un v√©lo"
				+ "\n4 : Infos bornes"
				+ "\n5 : RÔøΩserver un vÔøΩlo"
				+ "\n(0 : Quitter)\n");
		System.out.flush();
		...
		case 1:
			...
			if (b == true) {
				...
				try {
					...
					System.out.println("Votre inscription a √©t√© prise en compte votre code secret est : "+ codeSecret );
				} catch (SQLException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
			...
			break;
		case 2:
			System.out.println("Afin de louer un v√©lo veuillez entrer l'identifant de la station");
			String res;
			try {
				int velodispo=-1;
				int idVelo =0;
		...
		case 3:
			...
			while (valide==false) {
				...
				try {
					...
					//verifier velo
					System.out.println("Saisir l'id du v√©lo √† rendre : ");
					System.out.flush();
					...
					while(veloTrouve==false){
						System.out.println("Le v√©lo ne coresspond pas au v√©lo lou√© de nouveau : ");
						System.out.flush();
		...
		case 5:
			...
			while (!authentification_valide) {
				...
				if(!RequetesBD.isAbonne(conn.getConn(), id)){
					System.out.println("Il faut Ítre abonnÈ pour rÈserver");
				}
				System.out.println("Saisir votre code secret");
				System.out.flush();
				...
				authentification_valide= RequetesBD.identification(conn.getConn(), id, code_secret);
					...
					System.out.println("RÈserver un vÈlo ‡ une station \n");
					res = RequetesBD.afficherStations(conn.getConn());
						System.out.println(res);
						...
						System.out.flush();
						int idStation = LectureClavier.lireEntier(saisie);
						System.out.println("Choisir une date de rÈservation (jj-mm-aaaa hh:mm) \n")  ;
						System.out.flush();
						
						String date_location = LectureClavier.lireChaine();
						// TODO : Test date bien formÈe
